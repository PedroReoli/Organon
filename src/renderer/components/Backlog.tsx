...
export const Backlog = ({
  ...
  return (
    <aside className={`backlog ${isCollapsed ? 'is-collapsed' : ''}`}>
      <header className="backlog-header">
        <div className="backlog-header-left">
          {!isCollapsed ? <h2>Backlog</h2> : null}
          {!isCollapsed ? <span className="backlog-count">{cards.length}</span> : null}
        </div>

        <button
          type="button"
          className="backlog-toggle-btn"
          onClick={onToggleCollapsed}
          title={isCollapsed ? 'Expandir backlog' : 'Recolher backlog'}
          aria-label={isCollapsed ? 'Expandir backlog' : 'Recolher backlog'}
        >
