export const ReminderAlertModal = ({ isOpen, message, onClose }: { isOpen: boolean, message: string, onClose: () => void }) => {
    ...
    return (
        <div style={{ position: 'fixed', bottom: '24px', right: '24px', background: 'var(--color-surface)', padding: '16px', borderRadius: '8px', border: '1px solid var(--color-primary)', boxShadow: '0 4px 12px rgba(0,0,0,0.3)', zIndex: 1000 }}>
            ...
            <div style={{ marginTop: '12px', display: 'flex', gap: '8px' }}>
                ...
                <button onClick={onClose} style={{ flex: 1, padding: '4px', background: 'var(--color-primary)', border: 'none', color: 'white', borderRadius: '4px', cursor: 'pointer' }}>Snooze 15m</button>
            </div>
        </div>
    )
}
