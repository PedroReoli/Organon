...
export const SprintKanbanColumn = ({ title, tasks, onEdit }: { title: string, tasks: PlanningTask[], onEdit: (id: string) => void }) => {
    return (
        <div style={{ flex: 1, minWidth: '280px', background: 'var(--color-surface)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.04)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '8px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {tasks.map(task => (
                    <PlanningCardStandard key={task.id} task={task} onEdit={() => onEdit(task.id)} />
                ))}
            </div>
        </div>
    )
