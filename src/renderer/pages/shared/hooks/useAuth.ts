...
export interface UseAuthReturn {
  ...
  /** Módulos habilitados pela licença atribuída ao usuário. null = sem licença. */
...
export function useAuth(
  ...
  useEffect(() => {
    ...
    void (async () => {
      setIsRestoring(true)
      try {
        configureOrganon({ accessToken: persistedRefreshToken })
        const me = await organonApi.auth.me()
        setUser(me.data)
        setModules((me.data as unknown as { modules?: Record<string, boolean> | null }).modules ?? null)
        setIsAuthenticated(true)
        onSessionChange(persistedRefreshToken, me.data.email, '')
      } catch {
