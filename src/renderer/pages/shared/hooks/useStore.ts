...
export const useStore = () => {
  ...
  const addApp = useCallback((input: {
    name: string
    exePath: string
    ...
    return appsSlice.addApp(input)
  }, [appsSlice])

  const updateApp = useCallback((appId: string, updates: Partial<Pick<AppItem, 'name' | 'exePath' | 'iconPath' | 'args' | 'workingDir' | 'envVars' | 'groupId' | 'autoLaunch' | 'tags' | 'isFavorite' | 'isPinned' | 'description'>>) => {
    appsSlice.updateApp(appId, updates)
  }, [appsSlice])

  const recordAppLaunch = useCallback((appId: string | null, macroId: string | null) => {
    appsSlice.recordAppLaunch(appId, macroId)
  }, [appsSlice])

  const addAppGroup = useCallback((name: string, color = '#6b7280', parentId: string | null = null) => {
    return appsSlice.addAppGroup(name, color, parentId)
  }, [appsSlice])

  const updateAppGroup = useCallback((groupId: string, updates: Partial<Pick<AppGroup, 'name' | 'color' | 'isExpanded'>>) => {
    appsSlice.updateAppGroup(groupId, updates)
  }, [appsSlice])

  const removeAppGroup = useCallback((groupId: string) => {
    appsSlice.removeAppGroup(groupId)
  }, [appsSlice])

  const updateMacroSteps = useCallback((macroId: string, steps: MacroStep[]) => {
    appsSlice.updateMacroSteps(macroId, steps)
  }, [appsSlice])

  const removeApp = useCallback((appId: string) => {
    appsSlice.removeApp(appId)
  }, [appsSlice])

  const addMacro = useCallback((input: { name: string; appIds: string[]; mode: 'sequential' | 'simultaneous' }) => {
    return appsSlice.addMacro(input)
  }, [appsSlice])

  const updateMacro = useCallback((macroId: string, updates: Partial<Pick<AppMacro, 'name' | 'appIds' | 'mode'>>) => {
    appsSlice.updateMacro(macroId, updates)
  }, [appsSlice])

  const removeMacro = useCallback((macroId: string) => {
    appsSlice.removeMacro(macroId)
  }, [appsSlice])
  ...
  const addHabit = useCallback((input: Omit<Habit, 'id' | 'createdAt' | 'order'>) => {
    return habitsSlice.addHabit(input)
  }, [habitsSlice])

  const updateHabit = useCallback((habitId: string, updates: Partial<Omit<Habit, 'id' | 'createdAt'>>) => {
    habitsSlice.updateHabit(habitId, updates)
  }, [habitsSlice])

  const removeHabit = useCallback((habitId: string) => {
    habitsSlice.removeHabit(habitId)
  }, [habitsSlice])

  const addHabitEntry = useCallback((input: Omit<HabitEntry, 'id'>) => {
    habitsSlice.addHabitEntry(input)
  }, [habitsSlice])

  const updateHabitEntry = useCallback((entryId: string, updates: Partial<Omit<HabitEntry, 'id'>>) => {
    habitsSlice.updateHabitEntry(entryId, updates)
  }, [habitsSlice])

  const removeHabitEntry = useCallback((entryId: string) => {
    habitsSlice.removeHabitEntry(entryId)
  }, [habitsSlice])

  const toggleHabitArchived = useCallback((habitId: string) => {
    habitExtrasSlice.toggleHabitArchived(habitId)
  }, [habitExtrasSlice])

  const upsertHabitEntry = useCallback(
    (habitId: string, date: string, updates: Partial<Omit<HabitEntry, 'id' | 'habitId' | 'date'>>) => {
      habitExtrasSlice.upsertHabitEntry(habitId, date, updates)
    },
    [habitExtrasSlice],
  )
  ...
  const addInvestment = useCallback((input: Omit<Investment, 'id' | 'createdAt'>) => {
    return investmentsSlice.addInvestment(input)
  }, [investmentsSlice])

  const updateInvestment = useCallback((id: string, updates: Partial<Omit<Investment, 'id' | 'createdAt'>>) => {
    investmentsSlice.updateInvestment(id, updates)
  }, [investmentsSlice])

  const removeInvestment = useCallback((id: string) => {
    investmentsSlice.removeInvestment(id)
  }, [investmentsSlice])
