...
import { pushAllToApi, pullFromApi } from '../../../../api/sync'
...
export function SyncProgressModal({ settings }: Props) {
  const [status, setStatus] = useState<'pushing' | 'pulling' | 'done' | 'error'>('pushing')
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        // Carrega conteúdo das notas locais
        ...
        // 1. Envia todos os dados locais para a API, categoria por categoria
        await pushAllToApi(rawStore, noteContents, (p) => setProgress(p))

        // 2. Baixa todos os dados do servidor (pull completo desde 2020).
        //    Garante que dados de outros dispositivos sejam incorporados e que
        //    lastSyncAt use o serverTime — evitando reload duplo no startup sync.
        setStatus('pulling')
        const { store: pulled, noteContents: pulledContents, serverTime } = await pullFromApi()

        // Escreve conteúdo das notas vindas do servidor (apenas se não-vazias)
        await Promise.all(
          pulled.notes.map(async (note) => {
            const content = pulledContents.get(note.id) ?? ''
            if (content.trim()) {
              await window.electronAPI.writeNote(
                (note as { mdPath?: string }).mdPath ?? `notes/${note.id}.md`,
                content,
              ).catch(() => {})
            }
          }),
        ...
        // Merge: remoto prevalece para mesmo ID, dados só-locais são preservados
        ...
        const merged = {
          ...
          notes: mergeById(rawStore.notes, pulled.notes).map(note => {
            ...
            return {
              ...note,
              mdPath: local?.mdPath ?? (note as { mdPath?: string }).mdPath,
          ...
          // serverTime evita reload duplo: hasRemoteChanges(serverTime) = false após reload
          lastSyncAt: serverTime,
        }

        await window.electronAPI.saveStore(merged as any)
      ...
      <div style={{
        ...
        {status === 'pulling' && (
          <>
            ...
            <div style={{ fontSize: 13, color: 'var(--color-text-muted, #a6adc8)', marginBottom: 20 }}>
