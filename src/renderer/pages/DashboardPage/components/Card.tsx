...
export const CardItem = ({ card, isOverlay, compact, projectColor, onClick, onContextMenu }: CardItemProps) => {
  ...
  return (
    <div
      ...
      className={[
        ...
        card.isLocked && !isOverlay ? 'card-locked' : '',
      ].filter(Boolean).join(' ')}
    >
      {/* Title row: status icon + title + priority + lock */}
      <div className="card-title-row">
        ...
        {card.isLocked && (
          <span className="card-lock-icon" title="Card travado neste dia">
      ...
      <div className="card-indicators">
        ...
        {!card.hasDate && !compact && (
          <div className="card-indicator card-indicator-nodate">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M12 20h9" />
              <path d="M3 20h3" />
...
export const SortableCard = ({ card, compact, projectColor, onClick, onContextMenu }: SortableCardProps) => {
  const {
    attributes,
    listeners,
    setNodeRef,
