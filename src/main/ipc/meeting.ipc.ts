...
function observe(sender: WebContents) {
  ...
  sender.once('destroyed', () => {
    jobs.get(sender.id)?.forEach(job => job.abort())
    jobs.delete(sender.id)
    watchers.get(sender.id)?.()
...
export function registerMeetingResearchIpc() {
  ...
  ipcMain.handle('meeting-agent:run', async (event, req: MeetingResearchRequest) => {
    ...
    if (ownerJobs.size >= 1 || [...jobs.values()].reduce((sum, items) => sum + items.size, 0) >= 3) {
      throw new Error('Há uma pesquisa em andamento. Aguarde ou cancele antes de iniciar outra.')
    }
    const controller = new AbortController()
    ownerJobs.set(req.id, controller)
    jobs.set(event.sender.id, ownerJobs)
    try {
      return await researchMeeting(req, controller.signal, stage => {
        if (!event.sender.isDestroyed()) {
          event.sender.send('meeting-agent:progress', { id: req.id, stage })
        }
      })
    } finally {
      ownerJobs.delete(req.id)
    }
  })

  ipcMain.handle('meeting-agent:cancel', (event, id: string) => {
    const job = jobs.get(event.sender.id)?.get(id)
    job?.abort()
    return !!job
  })

  ipcMain.handle('meeting-agent:watch', (event, root?: string) => {
    watchers.get(event.sender.id)?.()
    ...
    const watcher = fs.watch(resolved, { recursive: true }, (_kind, filename) => {
      ...
      if (
        isFileSensitiveOrBinary(relative) ||
        !isPathSafe(
          resolved,
          fs.existsSync(path.join(resolved, relative))
            ? path.join(resolved, relative)
            : path.dirname(path.join(resolved, relative))
        )
      ) {
        return
      }
      ...
      timer = setTimeout(() => {
        if (!event.sender.isDestroyed()) {
          event.sender.send('meeting-agent:files-changed', {
            root: resolved,
            files: [...changed].slice(0, 20),
    ...
    watcher.on('error', () => {
      if (!event.sender.isDestroyed()) {
        event.sender.send('meeting-agent:files-changed', {
          root: resolved,
          files: [],
          error: 'O acompanhamento da pasta foi interrompido. Vincule novamente.',
        })
      }
    })
    watchers.set(event.sender.id, () => {
      if (timer) clearTimeout(timer)
      watcher.close()
    })
