...
import { loadStore, loadStoreFromPath, normalizeStore, saveStore } from './store'
import { getInstallerStatus, migrateToDedicatedStorage } from './storageMigration'
import { installWhisperModelBundle } from './whisperModelInstaller'
import { assessCatastrophicDataLoss } from './storageIntegrity'
...
export const registerCoreIpcHandlers = (): void => {
  ...
  // ── Git exec (correção de commits ruins) ──────────────────────────────────
  ...
  ipcMain.handle('reports:run', (_event, scriptPath: string): Promise<{ ok: boolean; output: string; error: string }> => {
    return new Promise((resolve) => {
      ...
      if (fs.existsSync(scriptPath)) {
        ...
        const devScript = path.resolve(__dirname, '..', '..', '..', '..', 'data', 'reports', 'generate_report.py')
