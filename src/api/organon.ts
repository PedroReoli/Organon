...
// â”€â”€ Estado de autenticaÃ§Ã£o (mÃ³dulo singleton) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/**
 * Upgrade 02: id estavel da instancia do client. Persistido em
 * localStorage para sobreviver a reload mas distinto entre janelas.
 * Enviado em todo request HTTP via X-Client-Id e usado pelo
 * RealtimeClient para filtrar o eco do proprio evento via WebSocket.
 */
...
// â”€â”€ HTTP core â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function doFetch<T>(path: string, init?: RequestInit, withAuth = true): Promise<T> {
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({}))
    console.error(`[Organon] ${init?.method ?? 'GET'} ${path} â†’ ${res.status}`)
...
/**
 * Upgrade 02: forca um refresh do JWT. Usado pelo RealtimeClient
 * quando o servidor emite auth_expired.
 */
...
async function doRefresh(): Promise<RefreshTokens> {
...
// â”€â”€ Tipos exportados â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export const organonApi = {
  ...
  pingDetailed: async (): Promise<PingDiagnostics> => {
    const attempts: Array<{ path: string; withAuth: boolean; url?: string }> = [
      { path: '/organon/health/ping', withAuth: false, url: `${_baseUrl}/api/v1/organon/health/ping` },
  ...
  // PREFERENCES
  preferences: {
    getPlanner: () => get<{ data: { planner_start_hour: number; planner_end_hour: number; planner_interval: number } }>('/preferences/planner'),
    savePlanner: (body: { planner_start_hour?: number; planner_end_hour?: number; planner_interval?: number }) =>
      put<{ data: { planner_start_hour: number; planner_end_hour: number; planner_interval: number } }>('/preferences/planner', body),
  },
  ...
  // MACROS (standalone â€” app_ids: string[], mode: 'sequential'|'simultaneous')
  macros: {
